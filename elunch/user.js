...
exports.list = function(req, res, next) {
    ...
    if (req.method == "POST") {
        ...
        var sql1 =
            "SELECT `order_supplier_name`, `order_no`,`order_name`, count(`order_no`) FROM `elunch_orders2` WHERE  `order_supplier_name` = 'Mucha' AND `order_date`='" +
            data_list +
        ...
        db.query(sql1, function(err, result) {
            ...
            var sql3 =
                "SELECT `order_supplier_name`, `order_no`,`order_name`, count(`order_no`) FROM `elunch_orders2` WHERE  `order_supplier_name` = 'Opoka' AND `order_date`='" +
                data_list +
                "'GROUP BY `order_no` ORDER BY `order_supplier_name`";
