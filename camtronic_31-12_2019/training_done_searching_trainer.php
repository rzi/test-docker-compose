...
<div class="eyebrow">
    ...
    <?php
        echo 'Weryfikacja zrealizowanych szkoleń (filtr po trenerze): '.'<br>'. '<br>';
        echo "<form action='' name='form' method='GET'>";
        echo 'wprodadź cyfrową część numeru SESA dla wybranego trenera:  ' ;
        echo "<input type='text' name='trainer_for_filter_name'  placeholder=''>"." ";
        echo "<input type='submit' value='filtruj po trenerze'>";
        echo "</form>";

        if(isset($_GET['trainer_for_filter_name'])) {
            // id index exists
            $b= $_GET['trainer_for_filter_name'];
           // echo "wartośc _get: ". $b;
        }
