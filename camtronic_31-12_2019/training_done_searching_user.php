...
<div class="eyebrow">
    ...
    <?php
        echo 'Weryfikacja zrealizowanych szkoleń (filtr po uczestniku szkolenia): '.'<br>'. '<br>';
        echo "<form action='' name='form' method='GET'>";
        echo 'wprodadź cyfrową część numeru SESA dla wybranego uczestnika szkolenia: ' ;
        echo "<input type='text' name='filter_by_user'  placeholder=''>";
        echo "<input type='submit' value='filtruj po uczestniku'>";
        echo "</form>";

        if(isset($_GET['filter_by_user'])) {
            // id index exists
            $b= $_GET['filter_by_user'];
           // echo "wartośc _get: ". $b;
        }
    ...
    ?>
        ...
        <div class="panel_left">
            <?php   
                ...
                echo "<tbody>";
                if($result = mysqli_query($link,"select DISTINCT * from spl_base WHERE id_sesa=$b ")){
