      ...
      return <React.Fragment>
        {this.props.cart.map(item =>
          <tr key={item.product.id}>
            ...
            <td>{item.product.price.toFixed(2).replace('.',',')} zł</td>
            <td>{(item.quantity * item.product.price).toFixed(2).replace('.',',')} zł</td>
            <td>
        ...
        <tr>
          ...
          <th colSpan="2">{this.props.cartPrice.toFixed(2).replace('.', ',')} zł</th>
        </tr>
      </React.Fragment>
    }
  }
